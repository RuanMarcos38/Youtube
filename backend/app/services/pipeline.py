...
def run_pipeline(job_id: int) -> None:
    ...
    try:
        ...
        video_path = download_video(source.original_url, work_dir)
        ...
        for index, candidate in enumerate(plan, start=1):
            ...
            clip = Clip(job_id=job.id,start_seconds=candidate.start,end_seconds=candidate.end,hook=candidate.hook,reason=candidate.reason,title=candidate.title,description=candidate.description,copy_text=candidate.copy,tags_json=json.dumps(candidate.tags, ensure_ascii=False),file_path=str(output_path.resolve()),subtitle_path=str(srt_path.resolve()),status="ready")
            db.add(clip)
            db.commit()
