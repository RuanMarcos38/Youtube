...
def test_unaudited_public_block_disables_public_account_direct_post():
    ...
    try:
        ...
        assert "Direct Post" in creator["public_posting_block_reason"]
        assert "não enviará" in creator["public_posting_block_reason"]
        assert "video.upload" not in creator["public_posting_block_reason"]
    finally:
        db.close()
